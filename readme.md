...
* **rules.md**: カレンダーデータの作成に関する全ての詳細ルール。
    * 5種類の出力ファイル（`weekday.txt`, `youbi.txt`, `kyureki_cal.txt`, `kyureki.txt`, `cell.txt`）のフォーマット定義。
...
生成された5種類のテキストファイルは、カレンダーアプリの各表示レイヤー（日付表示、行事表示、旧暦表示）、他用途向け一覧、年間行事予定表の1日1セル表示に取り込まれることを想定しています。
...
対象年・月を選んで「生成」を押すと、以下の5種類を作成します。
...
- 1か月分または12か月分の `weekday.txt`、`youbi.txt`、`kyureki_cal.txt`、`kyureki.txt`、`cell.txt` を生成。
